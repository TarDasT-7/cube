        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{route('course_category.store')}}">
                        {{csrf_field()}}
    ...
    <div class="modal fade" id="Modal-2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{route('course_sub_category.store')}}">
                        {{csrf_field()}}
                        <div class="col-md-12  mb-1">
                            <fieldset class="form-group">
                                <label for="basicInput">عنوان</label>
                                ...
                                @foreach($categories as $category)
                                    <tr >
                                        ...
                                        <td class="text-center">
                                            @foreach($category->course_sub_categories as $cat)
                                                {{$cat->name}}<br>
                                            @endforeach
                                        </td>
                                        <td class="text-center">
                                            <a class="btn btn-warning" href="{{route('course_category.edit', $category->id)}}">ویرایش</a>
                                        </td>
                                        <td class="text-center">
                                            <form  method="post" action="{{route('course_category.destroy', $category->id)}}">
                                                @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="example" class="table table-striped ">
                                ...
                                <tbody>
                                @foreach($sub_categories as $ccategory)
                                    <tr >
                                        <td class="text-center">{{$ccategory->name}}</td>
                                        <td class="text-center">{{$ccategory->course_category->name}}</td>
                                        <td class="text-center">
                                            <a class="btn btn-warning" href="{{route('course_sub_category.edit', $ccategory->id)}}">ویرایش</a>
                                        </td>
                                        <td class="text-center">
                                            <form  method="post" action="{{route('course_sub_category.destroy', $ccategory->id)}}">
                                                @csrf
                                                <input type="hidden" name="_method" value="DELETE">
                                                <button type="submit" class="btn btn-danger ">حذف</button>
                                            </form>
