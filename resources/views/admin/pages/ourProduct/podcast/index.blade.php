                    ...
                    <div class="table-responsive">
                        <table id="example" class="table table-striped ">
                            ...
                            @foreach($podcasts as $key=>$podcast)
                                 <tr>
                                     <td style="text-align: center"><img width="150" src="{{"/images/podcasts/".$podcast->image}}" ></td>
                                    <td class="text-center">{{$podcast->title}}</td>
                                    <td class="text-center">{{$podcast->category->title}}</td>
                                ...
                                <div class="modal fade" id="update-{{$key}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form method="post" action='{{route('podcast.update' , $podcast->id)}}' enctype="multipart/form-data">
                                            @method('patch')
                                            @csrf
                                                <div class="modal-body">
                                                    ...
                                                    <div class="col-md-12  mb-1">
                                                        <fieldset class="form-group">
                                                            <label >عنوان</label>
                                                            <input type="text" name="title" class="form-control" value="{{$podcast->title}}" placeholder="عنوان فایل خود را وارد کنید">
                                                        </fieldset>
                                                    </div>
                                                    ...
                                                    <div class="col-md-12  mb-1">
                                                        <fieldset class="form-group">
                                                            ...
                                                            <select class="select2-bg form-control" name="category"  id="category" data-bgcolor="success" data-bgcolor-variation="lighten-3" data-text-color="white">
                                                                @foreach($categories as $category)
                                                                    <option @if($category->id == $podcast->category_id) selected @endif value="{{$category->id}}">{{$category->title}}</option>
                                                                @endforeach
                                                            </select>
                                                        </fieldset>
                                                    </div>
